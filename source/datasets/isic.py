...
class ISICDataset(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.mode == 'train' and np.random.rand() <= self.prob_aug:
            ...
            # Brightness
            if np.random.rand() <= _p:
                _aux = 1 - (np.random.rand() * 0.3)
                img = TF.adjust_brightness(img, _aux)

            # Hue
            if np.random.rand() <= _p:
                _aux = np.random.rand() * 0.05
                img = TF.adjust_hue(img, _aux)

            # Contrast
            if np.random.rand() <= _p:
                _aux = 1 - (np.random.rand() * 0.6)
...
def get_isic_dataloaders (base_path, image_size, batch_size, num_workers=16, augmentation_prob=0.5):

    print("-" * 50)
    print("- Loading ISIC dataset..")

    dataset_train = ISICDataset(os.path.join(base_path, "train"),
    ...
    dataset_val = ISICDataset(os.path.join(base_path, "valid"),
    ...
    dataset_test = ISICDataset(os.path.join(base_path, "test"),
